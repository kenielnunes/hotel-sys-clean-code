...
def funcaoCadastra():
    ...
    tipoQuarto = input(
        "Insira o tipo do quarto S (Standard), D (Deluxe) ou P (Premium): "
    )
    ...
    while (
        ...
        print("\033[0;31mCampo incorreto!\033[m")
        tipoQuarto = input(
            "Os tipos de quartos são S, D ou P, por favor insira um destes: "
        )
...
def funcaoBusca():
    ...
    for i in listaClientes:
        insereCliente, insereCpf, nPessoas, tipoQuarto, valor, nDias, status, valida = (
            i.split(",")
        )
        clientes = {
            "nome": insereCliente,
...
def funcaoAltera(novaLista):
    ...
    for i in novaLista:
        arquivo.write(
            f"{i['nome']},{i['cpf']},{i['nPessoas']},{i['tipoQuarto']},{i['nDias']},{i['valor']},{i['status']},\n"
        )
    arquivo.close
...
def funcaoCheckIn():
    ...
    for i in novaLista:
        if cpf == i["cpf"]:
            ...
            reserva += 1
            print(
                f"Reserva n°:{reserva} - {i['nome']},{i['cpf']},{i['nPessoas']},{i['tipoQuarto']},{i['nDias']},{i['valor']},{i['status']},\n"
            )
...
def funcaoAlterarReserva():
    ...
    for dados in novaLista:
        if cpf == dados["cpf"]:
            reserva += 1
            print(
                f"Reserva n°:{reserva} - {dados['nome']},{dados['cpf']},{dados['nPessoas']},{dados['tipoQuarto']},{dados['nDias']},{dados['valor']},{dados['status']},\n"
            )
    selecionaReserva = int(input("Selecione a reserva que deseja alterar: "))
    reserva = 0
    for dados in novaLista:
        if cpf == dados["cpf"]:
            ...
            if reserva == selecionaReserva:
                ...
                dados["tipoQuarto"] = input(
                    "Tipo de quarto S (Standard), D (Deluxe) ou P (Premium): "
                ).upper()
                dados["status"] = input("Status do quarto: ").upper()
                if dados["tipoQuarto"] == "S":
                    dados["valor"] = (100 * dados["nPessoas"]) * dados["nDias"]
                elif dados["tipoQuarto"] == "D":
                    dados["valor"] = (100 * dados["nPessoas"]) * dados["nDias"]
                elif dados["tipoQuarto"] == "P":
                    dados["valor"] = (100 * dados["nPessoas"]) * dados["nDias"]
    funcaoAltera(novaLista)
