...
class ReservationRepository(BaseRepository[Reservation]):
    ...
    def create(self, reservation: Reservation) -> Reservation:
        ...
        print(
            f"Creating reservation with total value: R$ {reservation.total_value:.2f}"
        )
        cursor.execute(
            """
        ...
        print(
            f"Reservation created with ID {reservation.id} and total value: R$ {reservation.total_value:.2f}"
        )
        return reservation
    ...
    def find_by_customer_id(self, customer_id: int) -> List[Reservation]:
        cursor = self._get_cursor()
        cursor.execute(
            "SELECT * FROM reservations WHERE customer_id = ?", (customer_id,)
        )
    ...
    def update_status(
        self, id: int, status: ReservationStatus
    ...
    def _map_row_to_entity(self, row) -> Reservation:
        ...
        room_type_map = {"S": "Standard", "D": "Deluxe", "P": "Premium"}
        ...
        print(
            f"Mapped reservation {reservation.id} with total value: R$ {reservation.total_value:.2f}"
        )
        return reservation
