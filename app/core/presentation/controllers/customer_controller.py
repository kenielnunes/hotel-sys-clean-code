...
class CustomerController:
    def __init__(self):
        ...
        self._create_customer_use_case = CreateCustomerUseCase(
    ...
    ) -> CustomerSingleResponse | CustomerErrorResponse:
        try:
            # Create customer
            customer = self._create_customer_use_case.execute(
                name=request["name"], cpf=request["cpf"]
            )

            return {
                "status_code": 201,
                "body": {"id": customer.id, "name": customer.name, "cpf": customer.cpf},
            }
